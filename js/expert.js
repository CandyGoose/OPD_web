async function init(){
    ...
    const prof = await fetch("http://172.28.22.160:8888/backend/get_profession.php?id=" + $_GET['id']);
    console.log("http://172.28.22.160:8888/backend/get_profession.php?id=" + $_GET['id']);
    const prof_conv = await prof.json();
    // TODO: do error for no profession
    console.log(prof_conv['success'])
    if (prof_conv['success'] == true){
        window.location.replace("http://172.28.22.160:8888/error.php");
    }
    ...
    const res = await fetch("http://172.28.22.160:8888/backend/get_categories.php");
    ...
    for (const cat in conv) {
        ...
        //console.log("http://172.28.22.160:8888/backend/get_internals_by_category.php?category="+cur);
        const iternals = await fetch("http://172.28.22.160:8888/backend/get_internals_by_category.php?category="+cur);
